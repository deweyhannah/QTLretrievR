...
run_mediate <- function(peaks, mapping, suggLOD = 7, outdir, biomart, med_out){
  ...
  res_out <- BiocParallel::bplapply(names(qtl_peaks), function(tissue) qtl_mediate(tissue, QTL.peaks = qtl_peaks, med_annot = med_annot, QTL.mediator = qtl_mediatior,
                                                                                     targ_covar = targ_covar, QTL.target = qtl_target, probs = probs,
                                                                                     mapDat = map_dat2),
...
qtl_mediate <- function(tissue, QTL.peaks, med_annot, QTL.mediator, targ_covar, QTL.target, probs, mapDat){
  ...
  med_res <- BiocParallel::bplapply(1:(n.batches - 1), function(i) purrr::compact(batchmediate(batch = i, QTL.peaks = QTL.peaks[[tissue]],
