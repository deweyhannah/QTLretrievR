...
#'  compatible with `qtl2`, organized by tissue and sample, optionally saves the
#'   result as an RDS file
#'
#'
#' @param outfile File path to save the resulting genopbrobs list. Defaults to
#' "gbrs_interpolated_genoprobs.rds".
#' @param gbrsFileLoc File path to where the GBRS files are located.
#' @param tissues Character vector of tissue names. These should match
#' substrings in the GBRS file names. if empty, defaults fo "a".
#' @param gridFile Either a data frame or file path to the genome grid used
#' to assign marker names. Default is 75K grid loaded with the package.
#' @param save Character. Determines output behavior: "sr" to save and return,
#'  "so" to save only, "ro" to return only. Default is "sr".
#'
#' @return A named list of 3D genotype probability arrays (one per tissue),
...
genoprobably <- function(outfile = "./gbrs_interpolated_genoprobs.rds",
  ...
  if (file.exists(gbrs.interp)) {
    stop(paste0(outfile, " already exists. Please delete or use new name."))
  }

  ## Locate all interpolated genoprob TSV files in the specified directory
  filenames <- c(list.files(gbrsFileLoc,
    pattern = ".*.gbrs.interpolated.genoprobs.tsv$", full.names = TRUE,
    recursive = TRUE
  ))
  ...
  if (length(filenames) / length(tissues) < 1 & length(tissues) != 0) {
    stop(paste0(gbrsFileLoc, " does not contian enough files for the
                tissues indicated"))
  }

  ## Separate files by tissue type - tissue names provided should match how they
  ## are named in the samples
  tissue_files <- list()
  if (length(tissues) > 0) {
  ...
  tissue_samps <- list()
  for (tissue in tissues) {
    samples <- unlist(strsplit(basename(tissue_files[[tissue]]), ".",
                               fixed = TRUE))[[1]]
    for (i in 2:length(tissue_files[[tissue]])) {
      samples[i] <- unlist(strsplit(basename(tissue_files[[tissue]])[i],
                                    ".", fixed = TRUE))[[1]]
    }
  ...
  for (tissue in tissues) {
    ...
    for (i in 1:seq_len(length(tissue_samps[[tissue]]))) {
      tsvfile <- tissue_files[[tissue]][i]
      samp <- tissue_samps[[tissue]][i]
      genoprobs_list[[tissue]][[samp]] <- readr::read_tsv(tsvfile,
  ...
  ## Assign marker names to the third dimension of the 3D array using the
  ## genome grid
