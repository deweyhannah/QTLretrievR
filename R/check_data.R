...
#' @param type The name of the object if a single one is passed. Use "genoprobs"
#'  for genotype probabilities, "mediation" for mediation results and "peaks"
#'  for a table of eQTL peaks.
#' @return List of properly names R objects to be used in downstream analysis
#' of QTLretrievR.
#'
#'
...
check_data <- function(x, type = "") {
  ...
  if (is.character(x)) {
    y <- readRDS(x)
    if (length(intersect(names(y), c("probs_list", "map_dat2", "peaks_list",
                                     "res_list", "effects_blup"))) == 1) {
  ...
  if (is.list(x)) {
    ...
    if (length(intersect(names(x), c("probs_list", "map_dat2", "peaks_list",
                                     "res_list", "effects_blup"))) == 1) {
  ...
  if (type == "") {
    ...
    b <- data.frame(table(c(names(kinship_loco[[1]]), names(gmap), names(pmap),
                            names(qtlprobs[[1]]))))
    stopifnot(length(unique(b$Freq)) == 1)

    ## Sample Check
    for (tissue in names(tissue_samp)) {
      stopifnot(identical(sort(tissue_samp[[tissue]]$ID),
                          sort(colnames(expr_list[[tissue]]))))
      stopifnot(identical(sort(tissue_samp[[tissue]]$ID),
                          sort(rownames(exprZ_list[[tissue]]))))
      stopifnot(identical(sort(tissue_samp[[tissue]]$ID),
                          sort(rownames(covar_list[[tissue]]))))
      stopifnot(identical(sort(rownames(expr_list[[tissue]])),
                          sort(colnames(exprZ_list[[tissue]]))))
    }

    ## map_dat2 column check
    stopifnot(all(c("chr", "cM", "marker", "chrom", "pos_bp", "n") %in%
                    colnames(map_dat2)))

    return(tibble::lst(qtlprobs, covar_list, expr_list, exprZ_list,
                       kinship_loco, gmap, pmap, tissue_samp, map_dat2))
  }
  if (type == "peaks") {
    stopifnot(is.list(peaks_list))
    for (tissue in names(peaks_list)) {
      stopifnot(is.data.frame(peaks_list[[tissue]]))
      stopifnot(all(c("phenotype", "peak_chr", "lod", "ci_lo", "ci_hi") %in%
                      colnames(peaks_list[[tissue]])))
  ...
  if (type == "mediation") {
    ...
    for (tissue in names(res_list)) {
      stopifnot(all(c("target_id", "target", "qtl_lod", "qtl_chr", "mediator",
                      "mediator_id", "mediator_chr", "mediator_midpoint", "LOD")
                    %in% colnames(res_list[[tissue]])))
    }
    return(res_list)
  }
  if (type == "effects") {
